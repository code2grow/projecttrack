...
public class UserFactory {
	...
	public static Collection makeAllUsers(Department department) {
		Collection users = new LinkedList();
		users.add(makeProjectManager(department));
		users.add(makeSystemsManager(department));
		users.add(makeQAManager(department));
		users.add(makeDevelopmentManager(department));
		users.add(makeBusinessAnalyst(department));
		users.add(makeUpperManager());
		return users;
	}

	public static Collection makeAllUsers2(Department department) {
		Collection users = new LinkedList();
		users.add(new User("proj_mgr2", "John", "Doe", "faces",
				makeRoleTypeSet(RoleType.PROJECT_MANAGER,
						RoleType.PROJECT_APPROVER, RoleType.PROJECT_CREATOR),
				department));
		return users;
